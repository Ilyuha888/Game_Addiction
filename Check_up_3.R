...
gam %>% ggplot(aes(x=current_rating)) +
  ...
  geom_text(aes(x=mean(gam$current_rating)+1600, , color="red", label=paste0("Среднее"), y=0.00030)) +
  geom_text(aes(x=mean(gam$current_rating)+1600, , color="blue", label=paste0("Медиана"), y=0.00027)) +
  theme(legend.position = "none") + 
